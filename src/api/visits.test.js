...
describe('api',() => {
	describe('visits',() => {
		...
		describe('fetchVisits',() => {
			...
			it('some filters',() => {
				// return empty array on /visits request
				let requestUrl = baseUrl + '?country_code=US&latitude=35&query_type=nor';
				let mockReponse = [{
					"ip": "37.142.42.64",
					"city": "Tel Aviv",
					"country_code": "IL",
				...
				return visitsFunctions.fetchVisits({country_code : 'US', latitude : 35, query_type : 'nor'}).then(res => {
					expect(res.success).toBe(true);
			...
			it('on passing bad parameters',() => {
				...
				return visitsFunctions.fetchVisits({latitude : 'sdf'}).then(res => {
					...
					expect(res.err).toBe('Error parsing latitude, longitude, or metro_code');
				})
			})
		})
	})
